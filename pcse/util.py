...
astro_nt = namedtuple(
    "AstroResults", "DAYL, DAYLP, SINLD, COSLD, DIFPP, " "ATMTR, DSINBE, ANGOT"
)


def reference_ET(
    DAY, LAT, ELEV, TMIN, TMAX, IRRAD, VAP, WIND, ANGSTA, ANGSTB, ETMODEL="PM", **kwargs
):
...
def wind10to2(wind10):
    ...
    wind2 = wind10 * (log10(2.0 / 0.033) / log10(10 / 0.033))
    return wind2
...
class ConfigurationLoader(object):
    ...
    def __init__(self, config):
        ...
        try:
            loc = {}
            bytecode = compile(
                open(model_config_file).read(), model_config_file, "exec"
            )
            exec(bytecode, {}, loc)
...
def is_a_month(day):
    ...
    if day.month == 12:
        ...
        if day == datetime.date(day.year, day.month + 1, 1) - datetime.timedelta(
            days=1
        ):
            return True
    return False
...
def is_a_dekad(day):
    ...
    if day.month == 12:
        ...
        if day == datetime.date(day.year, day.month, 10):
            ...
            return True
        elif day == datetime.date(day.year, day.month + 1, 1) - datetime.timedelta(
            days=1
        ):
            return True
    return False
...
class _GenericSiteDataProvider(dict):
    ...
    def __init__(self, **kwargs):
            ...
            if par_name not in kwargs:
                ...
                if isinstance(par_range, set):
                    if v not in par_range:
                        msg = "Value for parameter '%s' can only have values: %s" % (
                            par_name,
                            par_range,
                        )
                        raise exc.PCSEError(msg)
        ...
        if kwargs:
            msg = (
                "Unknown parameter values provided to WOFOSTSiteDataProvider: %s"
                % kwargs
            )
            print(msg)
...
class WOFOST72SiteDataProvider(_GenericSiteDataProvider):
    ...
    _defaults = {
        "IFUNRN": (0, {0, 1}, int),
        "NOTINF": (0, [0.0, 1.0], float),
        "SSI": (0.0, [0.0, 100.0], float),
        "SSMAX": (0.0, [0.0, 100.0], float),
        "WAV": (None, [0.0, 100.0], float),
        "SMLIM": (0.4, [0.0, 1.0], float),
    }
    _required = ["WAV"]
