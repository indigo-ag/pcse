...
def _create_PCSE_tables(metadata):
    ...
    table_crop = Table(
        ...
        Column("crop_model", Integer, nullable=False),
        schema=None,
    )
    table_crop.create()

    table_crop_calendar = Table(
        "crop_calendar",
        metadata,
    ...
    table_crop_parameter_value = Table(
        ...
        Column("parameter_yvalue", NewFloat()),
        schema=None,
    )
    ...
    table_grid = Table(
        ...
        Column("distance_to_coast", NewFloat(), nullable=False),
        schema=None,
    )
    table_grid.create()

    table_grid_weather = Table(
        "grid_weather",
        metadata,
        Column("grid_no", Integer, primary_key=True, nullable=False),
        ...
        Column("snow_depth", NewFloat()),
        schema=None,
    )
    table_grid_weather.create()

    table_pywofost_output = Table(
        "pywofost_output",
        metadata,
        Column("grid_no", Integer, primary_key=True, nullable=False),
        Column("crop_no", Integer, primary_key=True, nullable=False),
    ...
    table_pywofost_unittest = Table(
        "wofost_unittest_benchmarks",
        metadata,
        Column("grid_no", Integer, primary_key=True, nullable=False),
        Column("crop_no", Integer, primary_key=True, nullable=False),
        Column("year", Integer, primary_key=True, nullable=False),
    ...
    table_soil_physical_group = Table(
        "soil_physical_group",
        metadata,
    ...
    table_variety_parameter_value = Table(
    ...
    table_data_for_assimilation = Table(
...
def migrate_db(target_dsn=None):
    ...
    if target_dsn is None:
        print(
            "No target_dsn specified, see docstring on migrate_db() function"
    ...
    try:
        ...
        print(
            "Unable to open connection to database, due to following exception:"
            " %s" % e.args[0]
        )
        return
    ...
    try:
        ...
        print(
            "Unable to open connection to pywofost demo database"
    ...
    try:
        ...
        print(
            "Migrating the database failed with the "
            "following exception:\n %s" % e.args[0]
        )
