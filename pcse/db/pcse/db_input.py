...
def fetch_cropdata(metadata, grid, year, crop):
    ...
    if (rows is not None) and (len(rows) == 1):
        ...
        logger.error(
            (
                "None or multiple crop definitions found for grid: %7i and crop "
                + "no: %3i and year: %5i"
            )
            % (grid, crop, year)
        )
        raise CropdataError(
            (
                "No crop definition found for grid: %7i and crop "
                + "no: %3i and year: %5i"
            )
            % (grid, crop, year)
        )
    ...
    for paramcode in parameter_codes_sngl:
        ...
        if len(rows) == 0:
            logger.error(
                ("No crop parameter value found for:" + "crop %s, parameter: %s")
                % (crop, paramcode)
            )
            raise CropdataError(
                ("No crop parameter value found for:" + "crop %s, parameter: %s")
            ...
            logger.error(
                ("Multiple crop parameter values found for:" + "crop %s, parameter: %s")
                % (crop, paramcode)
            )
            raise CropdataError(
                ("Multiple crop parameter values found for:" + "crop %s, parameter: %s")
    ...
    logger.debug(
        "Succesfully retrieved single value parameters "
    ...
    for paramcode in parameter_codes_mltp:
        ...
        if c == 0:
            ...
            logger.error(
                ("Single crop parameter value found for:" + "crop %s, parameter: %s")
                % (crop, paramcode)
            )
            raise CropdataError(
                ("Single crop parameter value found for:" + "crop %s, parameter: %s")
    ...
    for paramcode in parameter_codes_sngl:
        ...
        if c == 0:
            ...
            errstr = (
                "Multiple values found for: crop: %s, variety: %s, "
    ...
    for paramcode in parameter_codes_mltp:
        ...
        if c == 0:
            ...
            errstr = (
                "Single value found for: crop: %s, variety: %s, "
                + "parameter: %s which is supposed to be a single value."
            )
            logger.error(errstr % (crop, paramcode))
            raise CropdataError(errstr % (crop, paramcode))
            ...
            cropdata[paramcode] = value
    logger.debug(
        "Succesfully retrieved array parameters " + "from VARIETY_PARAMETER_VALUE TABLE"
    )
...
def fetch_soilparams(metadata, grid, soilgroup):
    ...
    for code_parname, db_parname in soil_parameters:
        r = select(
            [table_soil_pg],
            and_(
                table_soil_pg.c.soil_group_no == soilgroup,
                table_soil_pg.c.parameter_code == db_parname,
            ),
        ).execute()
        rows = r.fetchall()
        if len(rows) == 0:
    ...
    for soil_par in soil_parameters_mltp:
        ...
        r = select(
            ...
            order_by=[table_soil_pg.c.parameter_code],
        ).execute()
        rows = r.fetchall()
        nrows = len(rows)
        if nrows < 3:
            msg = (
                "Contains less than 3 (x,y) pairs for:"
                + "soilgroup %s, parameter: %s" % (soilgroup, soil_par)
            )
            raise SoildataError(grid, msg)

        value = array.array("d", [0.0] * (nrows * 2))
        for i, row in enumerate(rows):
            value[i * 2] = float(row.parameter_xvalue)
            value[(i * 2) + 1] = float(row.parameter_yvalue)
        soilparams[soil_par] = Afgen(
            value
        )  # @ToDo: bij definitie AfgenTrait() zie assimilation.py
        ...
        if soil_par == "SMTAB":
            ...
            NGAU = 5
            PGAU = array(
                [0.0469100770, 0.2307653449, 0.5, 0.7692346551, 0.9530899230]
            )  # sum(PGAU) = 2.5?
            WGAU = array(
                [0.1184634425, 0.2393143352, 0.2844444444, 0.2393143352, 0.1184634425]
            )  # sum(WGAU) = 0.9999999998 ~ 1
            ...
            value = array.array("d", [0.0] * (ilsm * 2))
            ...
            soilparams["MFPTAB"] = Afgen(
                value
            )  # @ToDo: bij definitie AfgenTrait() zie assimilation.py

            EquilTableLEN = 30
            value = array.array("d", [0.0] * EquilTableLEN)  # WaterFromHeight
            inversevalue = array.array("d", [0.0] * EquilTableLEN)  # HeightFromAir
            ...
            for i in range(2, EquilTableLEN, 2):
                ...
                MH0 = MH1
                MH1 = 2 * MH1
            soilparams["WaterFromHeight"] = Afgen(
                value
            )  # @ToDo: bij definitie AfgenTrait() zie assimilation.py
            soilparams["HeightFromAir"] = Afgen(
                inversevalue
            )  # @ToDo: bij definitie AfgenTrait() zie assimilation.py
...
def fetch_soildata_layered(metadata, grid):
    ...
    r = select(
        [table_soiltype.c.grid_no, table_soiltype.c.soil_type_no],
        table_soiltype.c.grid_no == grid,
    ).execute()
    rows = r.fetchall()
    if len(rows) != 1:
    ...
    r = select(
        [table_soillayer.c.thickness, table_soillayer.c.soil_group_no],
        table_soillayer.c.soil_type_no == soil_type_no,
        order_by=[table_soillayer.c.layer_no],
    ).execute()
    rows = r.fetchall()
    if len(rows) == 0:
    ...
    logger.info(
        "Succesfully retrieved soil layer parameter values for layered soil from database"
    )
...
def fetch_soildata(metadata, grid):
    ...
    cursor = select(
        [table_soil_layers.c.thickness, table_soil_layers.c.soil_group_no],
    ...
    if len(rows) == 0:
        ...
        msg = (
            "Number of soil layers > 1. Not possible for unlayered "
    ...
    for wofost_soil_par, db_soil_par in soil_parameters:
        r = select(
            [table_soil_pg],
            and_(
                table_soil_pg.c.soil_group_no == soil_group_no,
...
def fetch_sitedata(metadata, grid, year):
    ...
    try:
        ...
        r = select(
            [table_site], and_(table_site.c.grid_no == grid, table_site.c.year == year)
        ).execute()
        row = r.fetchone()
        r.close()
        if row is not None:
...
class EnsembleGridWeatherDataProvider(WeatherDataProvider):
    ...
    def _fetch_location_from_db(self, metadata):
        ...
        wdc = WeatherDataContainer(
            LAT=row.latitude, LON=row.longitude, ELEV=row.altitude
        )
    ...
    def _fetch_ensemble_weather_from_db(self, metadata, wdc):
        ...
        columns = [
            ...
            egw.c.member_id,
            egw.c.rainfall,
            gw.c.maximum_temperature,
            gw.c.minimum_temperature,
            gw.c.calculated_radiation,
            gw.c.vapour_pressure,
            gw.c.windspeed,
            gw.c.et0,
            gw.c.es0,
            gw.c.e0,
        ...
        if nrows < expected_rows:
            msg = (
                "Only %i records selected while %i expected "
                + "for grid %i (%i days X %i members)"
            )
            self.logger.warn(
                msg
                % (nrows, expected_rows, self.grid_no, self.timeinterval, nmember_ids)
            )
        else:
            msg = (
                "Retrieved ensemble weather data for grid %s between "
                + "%s and %s for %i members"
            )
            self.logger.info(
                msg % (self.grid_no, self.startdate, self.enddate, nmember_ids)
            )
    ...
    def _make_WeatherDataContainer(self, row, wdc):
        ...
        twdc.add_variable("RAIN", float(row.rainfall) / 10.0, "cm/day")
        twdc.add_variable("E0", float(row.e0) / 10.0, "cm/day")
        twdc.add_variable("ES0", float(row.es0) / 10.0, "cm/day")
        twdc.add_variable("ET0", float(row.et0) / 10.0, "cm/day")
        twdc.add_variable("IRRAD", float(row.calculated_radiation) * 1000.0, "J/m2/day")
