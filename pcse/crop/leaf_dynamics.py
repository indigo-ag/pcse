...
class WOFOST_Leaf_Dynamics(SimulationObject):
    ...
    *External dependencies:*
    ...
    def _set_variable_LAI(self, nLAI):
        ...
        SAI = self.kiosk["SAI"]
        PAI = self.kiosk["PAI"]
        adj_nLAI = max(nLAI - SAI - PAI, 0.0)
        adj_oLAI = max(oLAI - SAI - PAI, 0.0)
...
class CSDM_Leaf_Dynamics(SimulationObject):
    ...
    def _CSDM(self, daynr):
        ...
        LAI_growth = 1.0 / (1.0 + exp(-p.CSDM_B * (daynr - p.CSDM_T1))) ** 2
        ...
        if LAI < p.CSDM_MIN:
            msg = (
                "LAI of CSDM model smaller then lower LAI limit "
    ...
    def initialize(self, day, kiosk, parvalues):
        ...
        LAI = self._CSDM(1)
        self.states = self.StateVariable(
            kiosk, LAI=LAI, DAYNR=1, LAIMAX=self.params.CSDM_MIN, publish="LAI"
        )
