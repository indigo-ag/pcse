...
class NPK_Demand_Uptake(SimulationObject):
    ...
    **External dependencies**
    ...
    def calc_rates(self, day, drv):
        ...
        # N demand [kg ha-1]
        r.NdemandLV = (
            max(mc.NMAXLV * k.WLV - k.NamountLV, 0.0)
            + max(k.GRLV * mc.NMAXLV, 0) * delt
        )
        r.NdemandST = (
            max(mc.NMAXST * k.WST - k.NamountST, 0.0)
            + max(k.GRST * mc.NMAXST, 0) * delt
        )
        r.NdemandRT = (
            max(mc.NMAXRT * k.WRT - k.NamountRT, 0.0)
            + max(k.GRRT * mc.NMAXRT, 0) * delt
        )
        r.NdemandSO = max(mc.NMAXSO * k.WSO - k.NamountSO, 0.0)

        # P demand [kg ha-1]
        r.PdemandLV = (
            max(mc.PMAXLV * k.WLV - k.PamountLV, 0.0)
            + max(k.GRLV * mc.PMAXLV, 0) * delt
        )
        r.PdemandST = (
            max(mc.PMAXST * k.WST - k.PamountST, 0.0)
            + max(k.GRST * mc.PMAXST, 0) * delt
        )
        r.PdemandRT = (
            max(mc.PMAXRT * k.WRT - k.PamountRT, 0.0)
            + max(k.GRRT * mc.PMAXRT, 0) * delt
        )
        r.PdemandSO = max(mc.PMAXSO * k.WSO - k.PamountSO, 0.0)

        # K demand [kg ha-1]
        r.KdemandLV = (
            max(mc.KMAXLV * k.WLV - k.KamountLV, 0.0)
            + max(k.GRLV * mc.KMAXLV, 0) * delt
        )
        r.KdemandST = (
            max(mc.KMAXST * k.WST - k.KamountST, 0.0)
            + max(k.GRST * mc.KMAXST, 0) * delt
        )
        r.KdemandRT = (
            max(mc.KMAXRT * k.WRT - k.KamountRT, 0.0)
            + max(k.GRRT * mc.KMAXRT, 0) * delt
        )
        ...
        r.RNuptake = (
            max(0.0, min(r.Ndemand - r.RNfixation, k.NAVAIL, p.RNUPTAKEMAX))
            * NutrientLIMIT
        )
