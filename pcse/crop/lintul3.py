...
class Lintul3(SimulationObject):
    ...
    **Rate variables:**
    ...
    def initialize(self, day, kiosk, parvalues):
        ...
        self.rates = self.Lintul3Rates(self.kiosk, publish=["PEVAP", "TRAN", "RROOTD"])
    ...
    def calc_rates(self, day, drv):
        ...
        SLA = p.SLAC * p.SLACF(DVS) * exp(-p.NSLA * (1.0 - NNI))
        ...
        FRT, FLV, FST, FSO = self.dryMatterPartitioningFractions(
            p.NPART, r.TRANRF, NNI, FRTWET, FLVT, FSTT, FSOT
        )
        ...
        GLAI = self._growth_leaf_area(
            DTEFF, self.LAII, DELT, SLA, GLV, WC, DVS, r.TRANRF, NNI
        )
        ...
        DRRT = 0.0 if (DVS < p.DVSDR) else s.WRT * p.RDRRT
        RNLDLV = p.RNFLV * DLV
        RNLDRT = p.RNFRT * DRRT

        # relative totalGrowthRate rate of roots, leaves, stem
        # and storage organs.
        RWLVG, RWRT, RWST, RWSO = self.relativeGrowthRates(
            RGROWTH, FLV, FRT, FST, FSO, DLV, DRRT
        )
        ...
        NDEMLV = max(NMAXLV * s.WLVG - s.ANLV, 0.0)
        NDEMST = max(NMAXST * s.WST - s.ANST, 0.0)
        NDEMRT = max(NMAXRT * s.WRT - s.ANRT, 0.0)
        NDEMSO = max(p.NMAXSO * s.WSO - s.ANSO, 0.0) / p.TCNT
        ...
        NUPTR = (max(0.0, min(NDEMTO, s.TNSOIL)) * NLIMIT) / DELT
        ...
        CBALAN = (
            s.TGROWTH
            + (p.WRTLI + p.WLVGI + p.WSTI + p.WSOI)
            - (WLV + s.WST + s.WSO + s.WRT + s.WDRT)
        )

        NBALAN = (
            s.NUPTT
            + (self.ANLVI + self.ANSTI + self.ANRTI + self.ANSOI)
        ...
        if abs(NBALAN) > 0.0001:
            raise NutrientBalanceError(
                "Nitrogen un-balance in crop model at day %s" % day
            )
    ...
    def _calc_potential_evapotranspiration(self, drv):
        ...
        pevap = max(0.0, pevap)
        ptran = (1.0 - exp(-0.5 * self.states.LAI)) * ET0
        ptran = max(0.0, ptran)
        return pevap, ptran
    ...
    def _calc_actual_transpiration(self, PTRAN, WC):
        ...
        if p.WMFAC:
            if WC > WCCR:
                ...
                FR = limit(0.0, 1.0, (WC - p.WCWP) / (WCCR - p.WCWP))
        ...
        #  that affects crop totalGrowthRate (FR) is formulated as below:
            ...
            if WCCR <= WC <= p.WCWET:  # Between critical
                ...
                FR = limit(0.0, 1.0, (p.WCST - WC) / (p.WCST - p.WCWET))
    ...
    def _growth_leaf_area(self, DTEFF, LAII, DELT, SLA, GLV, WC, DVS, TRANRF, NNI):
        ...
        if (DVS < 0.2) and (LAI < 0.75):
            GLAI = (
                (LAI * (exp(p.RGRL * DTEFF * DELT) - 1.0) / DELT)
                * TRANRF
                * exp(-p.NLAI * (1.0 - NNI))
            )

        # ---- Growth at day of seedling emergence:
        if (LAI == 0.0) and (WC > p.WCWP):
            GLAI = LAII / DELT

        return GLAI

    def dryMatterPartitioningFractions(
        self, NPART, TRANRF, NNI, FRTWET, FLVT, FSTT, FSOT
    ):
        """
    ...
    def totalGrowthRate(self, DTR, TRANRF, NNI):
        ...
        p = self.params
        PARINT = 0.5 * DTR * (1.0 - exp(-p.K * self.states.LAI))
    ...
    def translocatable_N(self):
        ...
        s = self.states
        p = self.params
        ATNLV = max(0.0, s.ANLV - s.WLVG * p.RNFLV)
        ATNST = max(0.0, s.ANST - s.WST * p.RNFST)
    ...
    def deathRateOfLeaves(self, TSUM, RDRTMP, NNI, SLA):
        ...
        RDRDV = 0.0 if (TSUM < p.TSUMAG) else RDRTMP

        RDRSH = max(0.0, p.RDRSHM * (s.LAI - p.LAICR) / p.LAICR)
        RDR = max(RDRDV, RDRSH)

        if NNI < 1.0:
            DLVNS = s.WLVG * p.RDRNS * (1.0 - NNI)
            DLAINS = DLVNS * SLA
        else:
            DLVNS = 0.0
            DLAINS = 0.0
