...
class WofostBenchmarkRetriever:
    ...
    def __init__(self, dsn, crop, grid, mode="pp"):
        ...
        self.table_benchm = Table("wofost_unittest_benchmarks", meta, autoload=True)
        self.crop = crop
        self.grid = grid
        self.mode = mode
        self.member_id = 0
    ...
    def __call__(self, variable):
        ...
        if column_for_retrieval is not None:
            s = select(
                [t1.c.day, column_for_retrieval],
                and_(
                    t1.c.crop_no == self.crop,
...
class WofostOutputRetriever:
    ...
    def __init__(self, dsn):
        ...
        self.table_pwo = Table("sim_results_timeseries", db_metadata, autoload=True)
...
class WofostTestingTemplate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.BenchmarkRetriever = WofostBenchmarkRetriever(
            dsn=self.dsn, crop=self.crop, grid=self.grid, mode=self.mode
        )
    ...
    def run_benchmark(self, var_to_benchmark, precision):
        ...
        for day, benchmark in benchmark_data:
            ...
            self.assertAlmostEqual(diff, 0.0, precision, assertmsg)
            n_assert += 1
