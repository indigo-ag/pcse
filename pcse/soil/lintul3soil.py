...
class Lintul3Soil(SimulationObject):
    ...
    **State variables:**
    ...
    def calc_rates(self, day, drv):
        ...
        RAIN = cm2mm(
            drv.RAIN
        )  # cm  --> mm CORRECTION FOR NON-STANDARD cm in WOFOST-WEATHER
        ...
        DRAIN, RUNOFF, IRRIG = self._drainage_runoff_irrigation(
            RAIN, EVAP, TRAN, DELT, s.WA, ROOTD
        )
        ...
        WATBAL = (
            s.WA
            + (s.TRUNOF + s.TTRAN + s.TEVAP + s.TDRAIN)
    ...
    def _drainage_runoff_irrigation(self, RAIN, EVAP, TRAN, DELT, WA, ROOTD):
        ...
        DRAIN = limit(0.0, p.DRATE, (WA - WAFC) / DELT + (RAIN - EVAP - TRAN))
        RUNOFF = max(0.0, (WA - WAST) / DELT + (RAIN - EVAP - TRAN - DRAIN))

        if p.WMFAC:
            # If a soil is irrigated by flooding, : soil water content is
            # kept at saturation via "irrigation events".
            IRRIG = (
                max(0.0, (WAST - WA) / DELT - (RAIN - EVAP - TRAN - DRAIN - RUNOFF))
                if p.IRRIGF
                else 0.0
            )
        else:
            ...
            IRRIG = (
                max(0.0, (WAFC - WA) / DELT - (RAIN - EVAP - TRAN - DRAIN - RUNOFF))
                if p.IRRIGF
                else 0.0
            )
    ...
    def _soil_evaporation(self, RAIN, PEVAP, ROOTD, DELT):
        ...
        if RAIN >= 0.5:
            ...
            self.DSLR += 1.0
            EVSMXT = PEVAP * (sqrt(self.DSLR) - sqrt(self.DSLR - 1.0))
        ...
        AVAILF = min(1.0, (s.WA - WAAD) / (EVS * DELT)) if (EVS > 0) else 0.0
